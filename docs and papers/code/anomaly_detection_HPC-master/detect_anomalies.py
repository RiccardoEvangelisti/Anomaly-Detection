...
"""
start_time = datetime.datetime.strptime("2018-03-03 00:00:01", "%Y-%m-%d %H:%M:%S")
end_time = datetime.datetime.strptime("2018-05-30 05:00:01", "%Y-%m-%d %H:%M:%S")
...
_freqGov = {
    ...
    "davide18": "powersave",
    "davide19": "powersave",
    "davide26": "performance",
    "davide27": "performance",
    "davide28": "performance",
    "davide29": "performance",
    "davide30": "performance",
    "davide31": "performance",
    "davide32": "performance",
    "davide33": "performance",
    "davide34": "performance",
    "davide42": "powersave",
    "davide45": "powersave",
}
...
def correlation_autoencoder(
    node, df, timestamps, series_labels, gaps, idle_periods, active_idle=[], ts_idle=[], ts_noIdle=[]
):
    st_train_set = str(timestamps[0]).replace("-", "").replace(":", "").replace(" ", "_")
    et_train_set = str(timestamps[-1]).replace("-", "").replace(":", "").replace(" ", "_")
    ...
    test_idx = 0
    (x_train, x_test, test_train_or_else_idxs, df_noAnomalies, df_anomalies, test_noAnomalies, test_anomalies) = (
        util.split_dataset(df_tensor, ts_noIdle, _freqGov[node])
    )
    ...
    util.error_distribution_2_class_varyThreshold(
        test_noAnomalies, decoded_normal_test, df_anomalies, decoded_anomal, node, df_noAnomalies, decoded_normal
    )

    # plot errors
    decoded_features = autoencoder.predict(df_tensor)
    stats_res = util.evaluate_predictions(decoded_features, df_tensor, gaps)
    util.plot_errors_DL(
        stats_res,
        series_labels,
        timestamps,
...
def semi_supervised_ae_based(node, df_obj):
    ...
    res = correlation_autoencoder(node, df, timestamps, df.columns, gaps, idle_periods, [], ts_idle, ts_noIdle)
...
def main(argv):
    ...
    df_already_prepared, df_file = util.check_df(_data_dir, start_time, end_time, node, _remove_idle)

    if not df_already_prepared:
        print("Retrieve data")
        data_ipmi, data_bbb, data_occ = util.retrieve_data(start_time, end_time, node, _data_dir)
        phys_data = util.create_df(data_ipmi, data_bbb, data_occ)

        df = pd.DataFrame(phys_data[node])
        df = df.transpose()
        (df, timestamps, idle_periods, active_idle, ts_noIdle, ts_idle, scaler) = util.prepare_dataframe(
            df, _remove_idle
        )
        ...
        with open(df_file, "wb") as handle:
            pickle.dump(df_obj, handle, protocol=pickle.HIGHEST_PROTOCOL)
    else:
