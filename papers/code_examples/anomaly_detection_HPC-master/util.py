...
def load_data(data_file):
    if os.path.isfile(data_file):
        ...
        print("Couldn't open data file {}".format(data_file))
        return None
...
def evaluate_predictions(predicted, actual, gaps):
    ...
    for i in range(nb_samples):
        for j in range(nb_series):
            ...
            if actual[i][j] != 0:
                p_abs_errors[j].append((abs(predicted[i][j] - actual[i][j])) * 100 / actual[i][j])
            sp_abs_errors[j].append((abs(predicted[i][j] - actual[i][j])) * 100 / (predicted[i][j] + actual[i][j]))
...
def plot_errors_DL(
    ...
    ts_idle=[],
    test_idx=-1,
    freqGov="conservative",
    title="Error",
):
    if len(ts_noIdle) > 0:
        plot_errors_DL_fixIdle(
            stats_res,
            series_labels,
            timestamps,
            sampling_time,
    ...
    if len(samples_idleness) > 0:
        for i in range(len(cumul_errors_norm)):
            print("Error norm: %s; Idle: %s" % (cumul_errors_norm[i], samples_idleness[i]))
    ...
    if test_idx != -1:
        ...
        plt.axvline(x=test_timestamp, linewidth=2, linestyle="--", color="k")
    ...
    for st, et in gaps:
        ...
        dif = end - start
        rect = patches.Rectangle((start, 0), dif, 1, fill=False, hatch="/", color="grey")
        ax.add_patch(rect)
    for st, et in idle_periods:
        start = mdates.date2num(st)
        end = mdates.date2num(et)
        dif = end - start
        rect = patches.Rectangle((start, 0), dif, 1, fill=False, hatch="//", color="yellow")
        ax.add_patch(rect)
...
def is_in_timeseries(timestamps, ts, sampling_time):
    for ts_1, ts_2 in pairwise(timestamps):
        if ts_1 <= ts <= ts_2 and (ts_1 - ts_2).total_seconds() <= sampling_time:
            return True
    return False
...
def add_freq_govs_to_plot(freqGov, ax):
    ...
    for st, et in freqGov_periods:
        ...
        dif = end - start
        rect = patches.Rectangle((start, -0.002), dif, 0.012, fill=True, color=color)
        ax.add_patch(rect)
...
def retrieve_data(start_time, end_time, node, data_dir):
    ...
    ipmi_file += str(start_time).replace("-", "").replace(":", "").replace(" ", "_") + "_"
    ipmi_file += str(end_time).replace("-", "").replace(":", "").replace(" ", "_") + ".pickle"
    ...
    occ_file += str(start_time).replace("-", "").replace(":", "").replace(" ", "_") + "_"
    occ_file += str(end_time).replace("-", "").replace(":", "").replace(" ", "_") + ".pickle"
    ...
    bbb_file += str(start_time).replace("-", "").replace(":", "").replace(" ", "_") + "_"
    bbb_file += str(end_time).replace("-", "").replace(":", "").replace(" ", "_") + ".pickle"
...
def create_df(phys_data_ipmi, phys_data_bbb, phys_data_occ):
    ...
    for n in nodes:
        ...
        phys_data_ipmi[n] = collections.OrderedDict(sorted(phys_data_ipmi[n].items()))
        phys_data_bbb[n] = collections.OrderedDict(sorted(phys_data_bbb[n].items()))
        phys_data_occ[n] = collections.OrderedDict(sorted(phys_data_occ_fixed[n].items()))
...
def check_df(data_dir, start_time, end_time, node, remove_idle, unscaled=False):
    ...
    df_file += str(start_time).replace("-", "").replace(":", "").replace(" ", "_") + "_"
...
def prepare_dataframe(df, remove_idle, unscaled=False):
    ...
    continuous_features = [c for c in continuous_features if c not in ["time_stamp", "node_id"]]
    ...
    if remove_idle:
        ...
        if len(ts_noIdle) == 0:
            return (-1, [], -1, [], [], [], -1)
        if unscaled:
            df = preprocess_noScaling(df, categorical_features, continuous_features)
            scaler = None
        else:
            df, scaler = preprocess(df, categorical_features, continuous_features)
        columns_2_drop = ["time_stamp", "active"]
        df = drop_stuff(df, columns_2_drop)

        return (df, timestamps, idle_periods, active_idle, ts_noIdle, ts_idle, scaler)
    else:
        if unscaled:
            df = preprocess_noScaling(df, categorical_features, continuous_features)
            scaler = None
...
"""
    ...
    return (train, test, test_train_or_else_idxs, df_noAnomalies, df_anomalies, test_noAnomalies, df_anomalies)
...
def error_distribution_2_class_varyThreshold(
    ...
    for i in range(nn_samples):
        for j in range(nn_series):
            abs_errors_normal[j].append(abs(pred_normal[i][j] - actual_normal[i][j]))
            squared_errors_normal[j].append(
                (pred_normal[i][j] - actual_normal[i][j]) * (pred_normal[i][j] - actual_normal[i][j])
            )
    ...
    for i in range(na_samples):
        for j in range(na_series):
            abs_errors_anomal[j].append(abs(pred_anomal[i][j] - actual_anomal[i][j]))
            squared_errors_anomal[j].append(
                (pred_anomal[i][j] - actual_anomal[i][j]) * (pred_anomal[i][j] - actual_anomal[i][j])
            )
    ...
    for i in range(nn_all_samples):
        for j in range(nn_all_series):
            abs_errors_normal_all[j].append(abs(pred_normal_all[i][j] - actual_normal_all[i][j]))
    ...
    if debug:
        ...
        print(
            "All Classes Weighted: precision %f, recall %f, F-score %f"
...
def plot_error_distribution_singleNode(node, actual_N, pred_N, actual_A, pred_A, actual_N_all, pred_N_all):
    ...
    plt.hist(errors_normal, bins=80, color="b", alpha=0.8, label="Node {} - Normal".format(node))

    plt.hist(errors_anomal, bins=50, color="r", alpha=0.8, label="Node {} - Anomaly".format(node))
    color_idx += 1
